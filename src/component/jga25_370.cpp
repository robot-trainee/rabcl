...
:   motor_output_max_(motor_output_max), control_mode_(control_mode),
  encoder_count_sum_(0), pre_diff_(0.0), cmd_value_(0.0),
  act_pos_(0.0), act_vel_(0.0), pre_output_(0.0)
{
...
void JGA25_370::UpdateEncoder()
{
  ...
  act_pos_ = static_cast<double>(encoder_count_sum_) * count_to_pos_factor_;
  act_pos_ = std::fmod(act_pos_, 2.0 * M_PI);
  if (act_pos_ < 0) {
    act_pos_ += 2.0 * M_PI;
  }
  act_vel_ = -1.0 * static_cast<double>(encoder_count) * count_to_vel_factor_;
}
...
int16_t JGA25_370::CalcMotorPosOutput()
{
  ...
  if (std::abs(output) > motor_output_max_) {
    if (output >= 0) {
      output = static_cast<int16_t>(motor_output_max_);
    } else {
      output = -1.0 * static_cast<int16_t>(motor_output_max_);
    }
  }
  pre_output_ = output;
